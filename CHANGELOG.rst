...
* Add new argument ``--thread`` to :command:`ngs-hc` command. This argument will be used to set ``--native-pair-hmm-threads`` for HaplotypeCaller and ``-XX:ParallelGCThreads`` and ``-XX:ConcGCThreads`` for Java.
