...
- **pygff** : The pygff submodule is designed for working with GFF (and GTF) files. It implements ``pygff.GffFrame`` which stores GFF data as ``pandas.DataFrame`` to allow fast computation and easy manipulation. The submodule strictly adheres to the standard `GFF specification <https://github.com/The-Sequence-Ontology/Specifications/blob/master/gff3.md>`_.
